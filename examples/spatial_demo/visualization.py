...
class Visualizer:
    ...
    def visualize_step(
        ...
        economy_status: Optional[Dict] = None
    ):
        ...
        if economy_status and economy_status.get('enabled'):
            title += (
                f" | Employed: {economy_status['employed_agents']}/{len(agents)}"
                f" | Unemp: {economy_status['unemployment_rate']:.0%}"
            )
    ...
    def plot_statistics(
        ...
        has_economy = 'employed_agents' in stats and bool(stats['employed_agents'])
        num_plots = 2 + num_places + (1 if has_fire else 0) + (2 if has_economy else 0)
        ...
        if has_economy:
            steps = range(len(stats['employed_agents']))
            axes[plot_idx].plot(
                steps, stats['employed_agents'],
                'b-', alpha=0.8, label='Employed agents'
            )
            axes[plot_idx].plot(
                steps, stats['unemployed_agents'],
                'm-', alpha=0.8, label='Unemployed agents'
            )
            axes[plot_idx].set_xlabel('Step')
            axes[plot_idx].set_ylabel('Number of Agents')
            axes[plot_idx].set_title('Employment Under LLM Automation')
            axes[plot_idx].legend()
            axes[plot_idx].grid(True, alpha=0.3)
            max_agents = max(
                max(stats['employed_agents']),
                max(stats['unemployed_agents'])
            ) if stats['employed_agents'] else MAX_AGENTS_DISPLAY
            axes[plot_idx].set_ylim(0, max(MAX_AGENTS_DISPLAY, max_agents + 2))
            plot_idx += 1

            steps = range(len(stats['average_cash_balance']))
            axes[plot_idx].plot(
                steps, stats['average_cash_balance'],
                'g-', alpha=0.8, label='Average cash'
            )
            axes[plot_idx].plot(
                steps, stats['average_structure_credit_balance'],
                'c-', alpha=0.8, label='Average structure credits'
            )
            if 'structure_credits_issued' in stats and stats['structure_credits_issued']:
                axes[plot_idx].plot(
                    steps, stats['structure_credits_issued'],
                    color='orange', alpha=0.6, linestyle=':',
                    label='Credits issued this step'
                )
            axes[plot_idx].set_xlabel('Step')
            axes[plot_idx].set_ylabel('Average Balance / Flow')
            axes[plot_idx].set_title('Cash and Structure-Sustain Currency')
            axes[plot_idx].legend()
            axes[plot_idx].grid(True, alpha=0.3)
            plot_idx += 1
